...
class ApiTest extends TestCase
{
    ...
    public function api_webhook ()
    {
		...
		// createGraph
		$client->createGraph($user_name, $graph_id, $graph_name, $unit, $graph_type, $color);
		
		$webhook_hash = null;
		try {
			// createWebhook
			$response = $client->createWebhook($user_name, $graph_id, $type);
			$this->assertTrue($response['isSuccess'], 'API: createWebhook');
			
			$webhook_hash = $response['webhookHash'];
			
			// getWebhook
			$response = $client->getWebhooks($user_name);
			$this->assertInternalType('array', $response['webhooks'], 'API: getWebhook');
			
			// invokeWebhook
			$response = $client->invokeWebhook($user_name, $webhook_hash);
			$this->assertTrue($response['isSuccess'], 'API: invokeWebhook');
			...
			// deleteWebhook
			$response = $client->deleteWebhook($user_name, $webhook_hash);
			$this->assertTrue($response['isSuccess'], 'API: deleteWebhook');
		}
		$this->assertTrue(true);
    }
}